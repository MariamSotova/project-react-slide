...
export class ParallaxButton extends Component {
  constructor(props) {
    super(props);
    this.update = 6;
  ...
  onMouseMove = (e) => {
    if (this.update === 6) {
      const dx =
        e.clientX - this.boundingRect.left - this.boundingRect.width / 2;
      const dy =
        e.clientY -
      ...
      this.div.style.setProperty(
        "--rx",
        `${(-dy / 0.85) * this.props.parallaxScale}deg`
      );
      this.div.style.setProperty(
        "--ry",
        `${(dx / 4) * this.props.parallaxScale}deg`
      );
    ...
    this.styles = StyleSheet.create({
      button: {
        ...
        ":before": {
          ...
          transform:
            "translateY(var(--ty, 0)) rotateX(var(--rx, 0)) rotateY(var(--ry, 0)) translateZ(var(--tz, -1em))",
...
ParallaxButton.defaultProps = {
  parallaxScale: 1,
  backgroundStyle: {
    background: "linear-gradient(left, #FC5C7D, #6A82FB)",
    borderRadius: "8px",
